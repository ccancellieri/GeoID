...
class EventDrainTask(AsyncWriteDrainTaskProtocol):
    ...
    async def run(self, payload: TaskPayload) -> TaskReport:
        ...
        # terminal-write statements so connection overhead is paid once. The
        # factory normalizes the DSN (prefix + libpq ``sslmode=`` → asyncpg
        # ``ssl=``, so a Cloud SQL DSN doesn't raise "unexpected keyword
