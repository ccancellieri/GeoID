...
class TaskIngestionRequest(BaseModel):
    ...
    database_batch_size: Optional[int] = Field(default=None, description="Maximum number of records per write batch (defaults to 500 when unset). A batch is flushed when this row cap OR max_batch_memory_mb is reached, whichever comes first.")
    max_batch_memory_mb: int = Field(default=100, description="Approximate geometry memory budget (in MB) accumulated before a batch is flushed. Bounds peak memory for geometry-heavy sources independently of the row count: a batch flushes when either this budget or database_batch_size is reached.")
