...
def test_es_drivers_items_es_only_metadata_stays_pg():
    ...
    The ES collection READ entry is hint-gated (geometry_simplified): it is
    only activated when a request explicitly requests the simplified
    geometry path.  READ carries no per-entry failure policy under the
    lane model — PG stays authoritative purely by ordering (it is the
    untagged catch-all reader; ES is opt-in via an explicit hint).
    Collection WRITE is PG-only (system of record).  ES is appended to the
    INDEX lane (materialization + derived search) by self-registration at
    apply time.
    """
    b = _build_routing_bundle(RoutingPresetParams(drivers=RoutingDrivers.ES), catalog_id="c1")
    ...
    # Collection READ has ES (hint-gated) + PG (untagged catch-all).
    ...
    if "collection_elasticsearch_driver" in read_refs:
        ...
        assert Hint.METADATA in es_entry.hints, (
            "ES READ entry must be hint-gated (opt-in only), never the "
            "untagged default"
        )
    # ES is the collection INDEX lane (materialization + derived search).
    assert "collection_elasticsearch_driver" in _op_refs(coll, Operation.INDEX)
