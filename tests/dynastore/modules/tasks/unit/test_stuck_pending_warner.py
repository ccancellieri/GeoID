...
"""Unit tests for ``StuckPendingWarnerService``.

The service is a periodic read-only scan that logs WARN lines for tasks that
...
- when the query raises, the service logs and continues (doesn't crash the tick).
"""
...
async def test_warner_swallows_query_errors(caplog):
    """A failing query is logged but doesn't crash the tick."""
    fake_query = AsyncMock()
    fake_query.execute = AsyncMock(side_effect=RuntimeError("PG died"))
