...
async def test_collection_handler_appends_tenant_scoped_log(
    ...
    # search_logs filters it back out by catalog_id + collection_id so it
    # still surfaces under the collection-scoped /logs endpoint (#2749: no
    # PG partitions anymore, this is now a deliberate stream choice, not a
