...
async def test_apply_mapping_full_lifecycle_against_real_postgres(db_engine) -> None:
    ...
    mapping_id, rows = await store.apply_mapping(
        ...
        region_prop=column, aliases=[], unique_id_prop="FID", title="Countries",
    )
    try:
        ...
        with pytest.raises(UniqueViolationError):
            await store.apply_mapping(
                ...
                region_prop=column, aliases=[], unique_id_prop="FID", title="Conflict",
            )
    ...
    try:
        (mapping_id_a, rows_a), (mapping_id_b, rows_b) = await asyncio.gather(
            store.apply_mapping(
                engine_a,
                catalog_id=catalog_id, collection_id=collection_id,
                region_prop=column, aliases=[], unique_id_prop="FID", title="Race",
            ),
            store.apply_mapping(
                engine_b,
                catalog_id=catalog_id, collection_id=collection_id,
