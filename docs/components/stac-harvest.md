...
| `kind` | `VECTOR` \| `RASTER` \| `RECORDS` | — | Optional collection kind to pin before writing collection metadata. When omitted, harvest infers `RASTER` from the STAC Raster extension or raster/COG data assets and otherwise leaves the platform default in place. |
...
Item-level provider links that describe renderable resources, such as SLD style
or legend links, are preserved while navigation links (`self`, `root`,
`parent`, `collection`, pagination) are regenerated by the local API. This lets
the Maps and Tiles renderers reuse a source-provided stylesheet without copying
or registering the stylesheet as an internal style record.
